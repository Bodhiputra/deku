 ...
 *
 * Usage: node ops-hub/sync/shopify-pull.mjs finecoustic
 */
...
function shopifyGraphQL(query) {
  ...
  try {
    const raw = execSync(cmd, { encoding: 'utf8', cwd: join(__dirname, '..', '..', 'shopify') });
    return JSON.parse(raw);
  } catch (e) {
    console.error('Shopify CLI failed. Run: cd shopify && shopify auth login');
