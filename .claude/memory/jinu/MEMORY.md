...
- **Instagram session (Jinu-only):** `@christopherr1999` + Chrome profile **Fantechzoom** (Profile 16) — for **all** Instagram-related tasks. **Banned on IG:** `@finecoustic_global`, Finecoustic profile (Profile 19), Alex profile (Profile 17). Confirm logged-in username at session start; run `tools/ig-session-verify.sh --confirm` after snapshot — `chrome-mcp-guard.sh` blocks KOL profile URLs until verified.
